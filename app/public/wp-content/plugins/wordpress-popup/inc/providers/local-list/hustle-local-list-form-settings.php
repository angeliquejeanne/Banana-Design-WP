...
class Hustle_Local_List_Form_Settings extends Hustle_Provider_Form_Settings_Abstract {
	...
	public function first_step_callback( $submitted_data ) {
		...
		$step_html = Hustle_Provider_Utils::get_integration_modal_title_markup(
			...
			sprintf( __( 'Save the submissions in your database so you can access them or export them from the %1$sEmail Lists%2$s page. Local list (when active) also stores the status of active third party apps for each submission.', 'hustle' ),
					'<a href="' . $entries_page . '" target="_blank">', '</a>' )
		);
