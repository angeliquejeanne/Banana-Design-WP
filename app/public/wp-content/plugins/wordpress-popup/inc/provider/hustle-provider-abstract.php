...
abstract class Hustle_Provider_Abstract implements Hustle_Provider_Interface{
	 ...
	 *
	 * @since 3.0.5
	 * @since 4.0 Array containing multiple instances
	 *
	...
	final public function get_settings_wizard( $submitted_data, $module_id = 0, $current_step = 0, $step = 0 ) {
		...
		if ( ! is_array( $steps ) ) {
			return $this->get_empty_wizard( sprintf( __( 'No settings available for %1$s', 'hustle' ), $this->get_title() ) );
		}
	...
	final public function get_form_settings_wizard( $submitted_data, $module_id, $current_step = 0, $step = 0 ) {
		...
		if ( ! is_array( $steps ) ) {
			return $this->get_empty_wizard( sprintf( __( 'No Form Settings available for %1$s', 'hustle' ), $this->get_title() ) );
		}
		$total_steps = count( $steps );
		if ( $total_steps < 1 ) {
			return $this->get_empty_wizard( sprintf( __( 'No Form Settings available for %1$s', 'hustle' ), $this->get_title() ) );
	 ...
	 *
	 * @since 3.0.5
	 * @since 4.0 $module_id param added
	 *
	 * @param int $module_id
	 * @return array
	 */
	...
	private function strip_30_global_provider_settings( $v3_provider ) {
		...
		foreach ( $v3_provider as $item => $value ) {
			if ( in_array( $item, $global_provider_settings ) ) {
				continue;
			}
